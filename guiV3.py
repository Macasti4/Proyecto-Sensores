...
class App:
    ...
    def cargar_csv(self):
        ruta = filedialog.askopenfilename(
            filetypes=[("Archivo CSV", "*.csv")],
            title="Cargar vueltas desde CSV"
        )
        ...
        try:
            with open(ruta, mode="r", newline="", encoding="utf-8-sig") as archivo:
                ...
                for fila in reader:
                    try:
                        vuelta_id = int(float(fila["vuelta"]))
                        punto = int(float(fila["punto"]))

                        angulo = float(fila["angulo_deg"]) % 360.0
                        d_us = max(0, float(fila["ultrasonido_cm"]))
                        d_ir = max(0, float(fila["infrarrojo_cm"]))
                        d_laser = max(0, float(fila["laser_cm"]))
                        fusion = max(0, float(fila["fusion_cm"]))

                    except (ValueError, TypeError, KeyError):
                        filas_invalidas += 1
                        continue

                    if vuelta_id not in datos_por_vuelta:
                        datos_por_vuelta[vuelta_id] = []

                    datos_por_vuelta[vuelta_id].append({
                        "punto": punto,
                        "angulo": angulo,
                        "us": d_us,
                        "ir": d_ir,
        ...
        if self.vueltas:
            respuesta = messagebox.askyesnocancel(
                "Cargar CSV",
                "¿Quieres reemplazar las vueltas actuales?\n\n"
                "Sí = reemplazar\n"
                "No = agregar al final\n"
                "Cancelar = no cargar"
            )
        ...
        indice_primera_vuelta_importada = len(self.vueltas)

        for vuelta_id in sorted(datos_por_vuelta.keys()):
            puntos = sorted(
                datos_por_vuelta[vuelta_id],
                key=lambda p: p["punto"]
            )

            if len(puntos) < 2:
                filas_invalidas += len(puntos)
                continue

            angulos = [p["angulo"] for p in puntos]

            vuelta = {
                "nombre": f"Vuelta {len(self.vueltas) + 1} - {len(puntos)} puntos (CSV)",
                "angulos_deg": angulos,
                "thetas": [math.radians(a) for a in angulos],
                "us": [p["us"] for p in puntos],
                "ir": [p["ir"] for p in puntos],
                "laser": [p["laser"] for p in puntos],
                "fusion": [p["fusion"] for p in puntos]
            }

            self.vueltas.append(vuelta)

        if len(self.vueltas) == indice_primera_vuelta_importada:
            messagebox.showwarning(
                "Aviso",
                "El CSV no tenía vueltas con suficientes puntos para cargar."
            )
            return

        self.actualizar_combo_vueltas()

        self.combo_vueltas.current(indice_primera_vuelta_importada)
        self.mostrando_vuelta_guardada = True
        self.dibujar_vuelta(self.vueltas[indice_primera_vuelta_importada])

        mensaje = f"CSV cargado correctamente:\n{ruta}"

        if filas_invalidas > 0:
            mensaje += f"\n\nFilas ignoradas: {filas_invalidas}"

        messagebox.showinfo("CSV cargado", mensaje)
